...
A collection of automation tools to help with the toughest achievement grinds in Heroes of Science and Fiction.
...
1. Double-click hosf_grinding_toolkit.exe
2. Pick a tab, set your coordinates, press Start
3. Press ESC at any time to stop

### Option 2: Python scripts (if you want to tweak the code)

1. Install Python from python.org (check "Add to PATH")
2. Open a terminal and run: `pip install pyautogui pynput`
3. Open the "scripts" folder
4. Edit parameters inside the script you need
5. Run with: `python script_name.py`
...
- Coordinates depend on your screen resolution and game window. Use the Mouse Capture tab (or get_mouse_positions.py) to find the correct positions on YOUR screen.

- For faster movement animations, you can quadruple hero speed through the Windows Registry:
  1. Open regedit (Win+R, type regedit, Enter)
  2. Navigate to: `HKEY_CURRENT_USER\SOFTWARE\Oxymoron Games\Silence of the Siren`
  3. Find OPTION_EXPLORATION_ANIMATION_SPEED (the one without _AI_ in the name)
  4. Open it and change the 7th byte value to 30 for quadruple speed (hexadecimal)
  5. Restart the game

- All scripts are single-player automation only. No online, no multiplayer, nothing bannable.

- The -testtools launch option (add in Steam → right-click game → Properties → General → Launch Options) enables dev buttons: instant kill in combat, refill movement, grant resources. Very useful for many achievements.
